...
from models import (Cnn_5layers_AvgPooling, Cnn_9layers_MaxPooling, 
    Cnn_9layers_AvgPooling, Cnn_13layers_AvgPooling, GLU_Cnn_9layers_AvgPooling, Ring_9layers, ArcFace_9layers)
...
def train(args):
    ...
    if pretrained == 'none':
        ...
        validate_statistics_path = os.path.join(workspace, 'statistics', filename, 
            ...
            'train_source={}'.format(train_source), 'segment={}s,hop={}s,pad_type={}'.format(
            segment_seconds, hop_seconds, pad_type), 'holdout_fold={}'.format(holdout_fold), 
            model_type, 'resume', 'validate_statistics.pickle')
        create_folder(os.path.dirname(validate_statistics_path))
    ...
    iter_per_epoch = np.ceil(len(data_generator.train_segments_indexes) / batch_size).astype(int)
    ...
    if cuda:
        metric_loss.cuda()
    optimizer = Nadam(model.parameters(), lr=0.0035, betas=(0.9, 0.999), eps=1e-8,
        weight_decay=0, schedule_decay=4e-3)
    scheduler = CosineLRWithRestarts(optimizer, batch_size, 
        len(data_generator.train_segments_indexes), restart_period=epoch_to_flat-epoch_to_warm+1, t_mult=1, verbose=True)
    ...
    optimizer.zero_grad() # when using gradient accumulation
    for batch_data_dict in data_generator.generate_train():
        ...
        # Train
        model.train()
        batch_feature, batch_output = model(batch_data_dict['feature'], is_infer=False)
        
        # loss
        loss = criterion(batch_output, batch_data_dict['target']) + metric_loss(batch_feature, batch_data_dict['target'])

        # Backward (Comment out when using gradient accumulation)
        # optimizer.zero_grad()
        ...
        if (iteration + 1) % 4 == 0:
            # every 4 iterations of batches of size 32
            optimizer.step()
            optimizer.zero_grad()

        # Comment out when using gradient accumulation
        # optimizer.step()

        if iteration >= epoch_to_warm * iter_per_epoch and iteration < epoch_to_flat * iter_per_epoch:
            if data_generator.pointer >= len(data_generator.train_segments_indexes):
                scheduler.step()
...
def get_train_features(args):
    ...
    feature_map_path = os.path.join(workspace, 'feature_map', filename, 
        'logmel_{}frames_{}melbins'.format(frames_per_second, mel_bins), 
        'train_source={}'.format(train_source), 'segment={}s,hop={}s,pad_type={}'
        ''.format(segment_seconds, hop_seconds, pad_type), 'holdout_fold={}'
    ...
    generate_func = data_generator.generate_validate('train_feature_map', train_source)

    # Results of segments
    output_dict = forward_dist(
        model=model, 
...
def get_infer_features(args):
    ...
    if infer_set == 'valid':
        data_generator = DataGenerator(
            ...
            scalar=scalar, 
            batch_size=batch_size)
        generate_func = data_generator.generate_validate('validate', train_source)
    ...
    infer_features = segment_feature_to_clip_feature(
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--pretrained', type=str, default='none', help='Resume training from given pretrained model')
    ...
    parser_get_train_features = subparsers.add_parser('get_train_features')
    parser_get_train_features.add_argument('--train_source', type=str, choices=['curated', 'noisy', 'curated_and_noisy'], required=True)
    parser_get_train_features.add_argument('--segment_seconds', type=float, required=True, help='Segment duration for training.')
    parser_get_train_features.add_argument('--hop_seconds', type=float, required=True, help='Hop duration between segments.')
    parser_get_train_features.add_argument('--pad_type', type=str, choices=['constant', 'repeat'], required=True, help='Pad short audio recordings with constant silence or repetition.')
    parser_get_train_features.add_argument('--holdout_fold', type=str, choices=['1', '2', '3', '4', '5', 'none'], required=True, help='Set `none` for training on all data without validation.')
    parser_get_train_features.add_argument('--model_type', type=str, required=True, help='E.g., Cnn_9layers_AvgPooling.')
    parser_get_train_features.add_argument('--iteration', type=int, required=True, help='Load model of this iteration.')
    parser_get_train_features.add_argument('--batch_size', type=int, required=True)
    parser_get_train_features.add_argument('--cuda', action='store_true', default=False)
    parser_get_train_features.add_argument('--mini_data', action='store_true', default=False, help='Set True for debugging on a small part of data.')

    # Obtain infer features
    parser_get_infer_features = subparsers.add_parser('get_infer_features')
    parser_get_infer_features.add_argument('--train_source', type=str, choices=['curated', 'noisy', 'curated_and_noisy'], required=True)
    parser_get_infer_features.add_argument('--segment_seconds', type=float, required=True, help='Segment duration for training.')
    parser_get_infer_features.add_argument('--hop_seconds', type=float, required=True, help='Hop duration between segments.')
    parser_get_infer_features.add_argument('--pad_type', type=str, choices=['constant', 'repeat'], required=True, help='Pad short audio recordings with constant silence or repetition.')
    parser_get_infer_features.add_argument('--holdout_fold', type=str, choices=['1', '2', '3', '4', '5', 'none'], required=True, help='Set `none` for training on all data without validation.')
    parser_get_infer_features.add_argument('--model_type', type=str, required=True, help='E.g., Cnn_9layers_AvgPooling.')
    parser_get_infer_features.add_argument('--iteration', type=int, required=True, help='Load model of this iteration.')
    parser_get_infer_features.add_argument('--batch_size', type=int, required=True)
    parser_get_infer_features.add_argument('--infer_set', type=str, choices=['valid', 'test'], required=True, help='Choose the inference dataset')
    parser_get_infer_features.add_argument('--cuda', action='store_true', default=False)
    parser_get_infer_features.add_argument('--mini_data', action='store_true', default=False, help='Set True for debugging on a small part of data.')
