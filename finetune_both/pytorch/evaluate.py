...
from utilities import (get_filename, inverse_scale, 
    segment_prediction_to_clip_prediction, embed_feature_to_clip_prediction)
from pytorch_utils import forward, forward_infer
...
class DistEvaluator(object):
    ...
    def evaluate(self, target_source, max_iteration=None, 
        verbose=False):
        ...
        train_generate_func = self.data_generator.generate_validate(
            data_type='train', 
            target_source=target_source, 
            max_iteration=max_iteration)

        valid_generate_func = self.data_generator.generate_validate(
            data_type='validate', 
            target_source=target_source, 
            max_iteration=max_iteration)
        
        # Results of segments
        train_output_dict = forward_dist(
            model=self.model, 
            generate_func=train_generate_func, 
            cuda=self.cuda, 
            return_target=True)

        valid_output_dict = forward_dist(
            model=self.model, 
            generate_func=valid_generate_func, 
            cuda=self.cuda, 
            return_target=True)
        
        # Results of audio recordings
        result_dict = embed_feature_to_clip_prediction(
            train_output_dict, valid_output_dict)
